 ...
 * std_font.cpp
 *
 * Contains standard font (originally copied from MD_MAX72XX system font)
 * and DMD_MAX72xx library implementation for characters and text processing.
 *
...
bool DMD_MAX72xx::setFont(font_t *f) {
  if (f == nullptr) return false;
  _fontInfo.firstAscii = pgm_read_byte(f);
  _fontInfo.lastAscii  = pgm_read_byte(f + 1);
  _fontInfo.width      = pgm_read_byte(f + 2);
  _fontInfo.height     = pgm_read_byte(f + 3);
  _fontInfo.data       = f + 4;
  return true;
}

DMD_MAX72xx::font_t *DMD_MAX72xx::getChrPtr(unsigned char chr) {
  uint8_t w, i(_fontInfo.firstAscii);
  font_t *chrptr(_fontInfo.data);
  if (chr < _fontInfo.firstAscii || chr > _fontInfo.lastAscii)
    return nullptr;
  if (_fontInfo.width == 0) {
    while (i < chr) {
      w = pgm_read_byte(chrptr++);
      chrptr += w;
      i++;
    }
  }
  else chrptr += (chr - _fontInfo.firstAscii)*_fontInfo.width;
  return chrptr;
}

uint8_t DMD_MAX72xx::getChrWidth(unsigned char chr) {
  uint8_t w(_fontInfo.width);
  if (w == 0) {
    font_t *chrptr = getChrPtr(chr);
    w = (chrptr != nullptr) ? pgm_read_byte(chrptr) : 0;
  }
  return w;
}

uint8_t *DMD_MAX72xx::copyChr(unsigned char chr) {
  font_t *chrptr(getChrPtr(chr));
  uint8_t w, i;
  if (chrptr == nullptr) return nullptr;
  w = pgm_read_byte(chrptr);
  if (w == 0) return nullptr;
  uint8_t *buf = (uint8_t *)calloc(4 + w, sizeof(uint8_t));
  buf[0] = w;
  buf[2] = _fontInfo.height;
  for (i = 0; i < w; i++)
    buf[4 + i] = pgm_read_byte(++chrptr);
  return buf;
}

uint8_t DMD_MAX72xx::drawChr(int16_t x, int16_t y, unsigned char chr) {
  uint8_t *buf(copyChr(chr));
  uint8_t w(0);
  if (buf != nullptr) {
    w = buf[0];
    if (_displayArea.left < x + w && x <= _displayArea.right && _displayArea.top < y + _fontInfo.height && y <= _displayArea.bottom)
      paste(x, y, buf);
    free(buf);
  }
  return w;
...
uint16_t DMD_MAX72xx::getStrWidth(unsigned char *str) {
  uint8_t strsz(strlen((const char *)str)), chrw, i;
  uint16_t strw(0);
  if (strsz > 0) {
    if (_fontInfo.width == 0) {
      for (i = 0; i < strsz; i++) {
        chrw = getChrWidth(str[i]);
        if (chrw > 0) strw += chrw + _fontSpacing;
      }
      strw -= _fontSpacing;
    }
    else strw = strsz*_fontInfo.width + (strsz - 1)*_fontSpacing;
  }
  return strw;
}

uint16_t DMD_MAX72xx::getStrWidth_P(const char *pstr) {
  unsigned char chr;
  uint8_t chrw, i(0);
  uint16_t strw(0);
  while (true) {
    chr = pgm_read_byte(pstr + i++);
    if (chr == 0) {
      if (strw > 0) strw -= _fontSpacing;
      return strw;
    }
    chrw = getChrWidth(chr);
    if (chrw > 0) strw += chrw + _fontSpacing;
  }
}

void DMD_MAX72xx::drawStr(int16_t x, int16_t y, unsigned char *str) {
  uint8_t strsz(strlen((const char *)str)), chrw, i;
  int16_t xi(x);
  if (strsz == 0 || y + _fontInfo.height <= _displayArea.top && _displayArea.bottom < y) return;
  for (i = 0; i < strsz - 1; i++) {
    chrw = drawChr(xi, y, str[i]);
    if (chrw > 0) {
      xi += chrw;
      drawFontSpacing(xi, y);
      xi += _fontSpacing;
    }
    if (_displayArea.right < xi) return;
  }
  drawChr(xi, y, str[i]);
}

void DMD_MAX72xx::drawStr_P(int16_t x, int16_t y, const char *pstr) {
  unsigned char chr;
  uint8_t chrw, i(0);
  int16_t xi(x);
  if (y + _fontInfo.height <= _displayArea.top && _displayArea.bottom < y) return;
  while (true) {
    chr = pgm_read_byte(pstr + i++);
    if (chr == 0) return;
    chrw = drawChr(xi, y, chr);
    if (chrw > 0) {
      xi += chrw;
      drawFontSpacing(xi, y);
      xi += _fontSpacing;
...
void DMD_MAX72xx::drawFontSpacing(int16_t x, int16_t y) {
  for (uint8_t i = 0; i < _fontSpacing; i++)
    drawVerticalLine(x, y, _fontInfo.height - 1, false);
}

bool DMD_MAX72xx::marquee(area_t a, unsigned char *str, const char *pstr, uint16_t delayTime, uint8_t times, bool (*callback)(void)) {
  area_t oldDispArea = getDisplayArea();
  uint8_t c((times > 0) ? times : 1);
  int16_t x, strWidth((pstr == nullptr) ? getStrWidth(str) : getStrWidth_P(pstr));
  uint32_t timer;
  setDisplayArea(a);
  while (c > 0) {
    for (x = a.right; a.left - strWidth <= x; x--) {
      if (pstr == nullptr)
        drawStr(x, a.top, str);
      else
        drawStr_P(x, a.top, pstr);
      drawVerticalLine(x + strWidth, a.top, _fontInfo.height, false);
      refresh();
      timer = millis();
#ifdef ESP8266
      ESP.wdtFeed();
...
DMD_MAX72xx::font_t PROGMEM _stdFont[] = {
  1, 255, 0, 8,
